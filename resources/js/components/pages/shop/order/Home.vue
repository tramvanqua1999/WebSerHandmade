                                                ...
                                                <div class="card">
                                                    ...
                                                    <div class="card-body">
                                                        ...
                                                        <thead>
                                                            ...
                                                            <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">Cancel</th>
                                                            ...
                                                            <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">Confirm</th></tr>
                                                        </thead>
                                                        <tbody>
                                                        ...
                                                        <td v-else class=""><button class="btn btn-default"> NOT Cancel</button></td>
                                                        <td class="sorting_1"><button class="btn btn-success"  @click.prevent="confirm(data.id)">Confirm</button></td>
                                                        </tr>
                                                        </tbody>
                                                    </table>
                                                        </div>
...
export default {
    ...
    methods: {
        refreshData() {
            let uri = `http://127.0.0.1:8000/api/shop/post/order/${localStorage.getItem('id')}`;
        ...
        cancel(id){
             let uri = `http://127.0.0.1:8000/api/shop/post/cancel/${id}`;
            this.axios.post(uri).then(response => {
                this.datas.splice(
        ...
        confirm(id) {
            let uri = `http://127.0.0.1:8000/api/shop/post/confirm/${id}`;
            this.axios.post(uri).then(response => {
                this.datas.splice(
                    this.datas.map(item => item.id).indexOf(id),
                    1
